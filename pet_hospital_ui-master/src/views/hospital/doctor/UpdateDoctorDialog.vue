<template>
  <div>
    <el-dialog
      ...
      @close="$emit('cancel')"
    >
      <el-form ref="doctorUpdateForm" :model="doctor" :rules="rules" label-width="30%" class="demo-ruleForm">
        <el-row>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleUpdateDoctor">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleUpdateDoctor() {
      this.$refs['doctorUpdateForm'].validate((valid) => {
        if (valid) {
          updateDoctor(this.doctor).then(res => {
            this.$message.success(res.body.data)
            this.$emit('updateDoctorSuccess')
          })
        } else {
          return false
        }
